...
const defaultConfig = {
    ...
    retryAttempts: 3,
    retryDelay: 1000,
    enableLogging:  true //import.meta.NODE_ENV === 'development'
}
...
class ApiService {
    ...
    async get(url, params = {}, options = {}) {
        //const signal = this.createSignal(url);
    ...
    createSignal(key) {
        ...
        if (this.signals.has(key)) {
            console.log(this.signals.get(key));
            this.signals.get(key)?.cancel('Request cancelled due to new request');
        }
    ...
    cancelRequest(key) {
        if (this.signals.has(key)) {
            this.signals.get(key)?.cancel('Request cancelled by user');
    ...
    cancelAllRequests() {
        this.signals.forEach((signal, key) => {
            signal.cancel('All requests cancelled');
        });
        this.signals.clear();
    }
